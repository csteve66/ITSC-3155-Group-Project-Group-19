...
@router.get("/sandwich/{sandwich_id}", response_model=list[schema.Recipe])
def read_by_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    return controller.read_by_sandwich(db, sandwich_id=sandwich_id)
